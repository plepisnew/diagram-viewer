...
export const RootControl: React.FC = () => {
	...
	const handleClickModel: MouseEventHandler<HTMLButtonElement> = async () => {
		try {
			setIsLoading(true);
			const loadingToastId = RootContext.procedures.triggerToast(
				toast.loading,
			...
			RootContext.apiClient.requestModel(
				...
				({ diagramUrl, model }) => {
					...
					if (diagramUrl) {
						...
						RootContext.procedures.triggerToast(
							toast.error,
							"A model was generated, but it could not be rendered"
						);
					}
	...
	const handleClickPrefillInput: MouseEventHandler<HTMLButtonElement> = async () => {
		RootContext.procedures.triggerToast(
			toast.success,
			"Imported 12 Software Requirements for Spotify's music player"
		);
					...
					<Box className={cn("p-4 h-full", "flex flex-col gap-4")}>
						...
						<div className={cn("flex [&>button]:flex-1 gap-4")}>
							<Button
								...
								disabled={isLoading || (!context && !userInput)}
							>
								Clear
							</Button>
